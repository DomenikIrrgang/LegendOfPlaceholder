...
func get_parry_chance() -> float:
	return 5.0 + (unit.stats.get_stat(Stat.Enum.PARRY) + unit.stats.get_stat(Stat.Enum.DEFENSE) * 0.25) / 20.0

func get_critical_receive_chance() -> float:
	return unit.stats.get_stat(Stat.Enum.DEFENSE) / 20.0

func get_miss_chance() -> float:
	return 5.0 + unit.stats.get_stat(Stat.Enum.MISS) / 20.0
	
func get_hit_chance() -> float:
	return (unit.stats.get_stat(Stat.Enum.HIT) + unit.stats.get_stat(Stat.Enum.INTELLECT) * 0.1) / 20.0
...
func get_critical_chance() -> float:
	return 5.0 + (unit.stats.get_stat(Stat.Enum.CRIT) * 1.5 + unit.stats.get_stat(Stat.Enum.INTELLECT) * 0.25) / 20.0
	
func get_critical_effect() -> float:
	return 1.5 + (unit.stats.get_stat(Stat.Enum.CRITICAL_EFFECT) * 0.05 + unit.stats.get_stat(Stat.Enum.AGILITY) * 0.02) / 20.0
...
func get_resistance(spell_school: SpellSchool.Enum) -> float:
...
func get_movement_speed() -> float:
	return unit.stats.get_stat(Stat.Enum.MOVEMENT_SPEED) + unit.stats.get_stat(Stat.Enum.HASTE) * 0.025
