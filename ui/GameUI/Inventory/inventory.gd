...
func on_slot_input(event: InputEvent, _slot: int) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_MASK_RIGHT and event.is_pressed():
		if inventory.slots[_slot].item_instance is GearInstance:
			var gear_instance = inventory.slots[_slot].item_instance
			var equiped_gear = Globals.get_player().gear_slots[gear_instance.item.slot]
			if equiped_gear != null:
				inventory.change_slot(_slot, equiped_gear, 1)
			else:
				inventory.change_slot(_slot, null, 0)
			Globals.get_player().equip_gear_in_slot(gear_instance.item.slot, gear_instance)
		else:
			inventory.use_slot(Globals.get_player(), _slot)

func pick_item(_slot: int) -> void:
	if Globals.get_drag_and_drop().is_dragging() == false and inventory.slots[_slot].item_instance != null:
		slots.get_child(_slot).select()
		selected_item = _slot
		Globals.get_drag_and_drop().start_dragging({
			"inventory": inventory,
			"inventory_slot": inventory.slots[_slot],
		}, inventory.slots[_slot].item_instance.item.icon)
...
func drop_item(_slot: int) -> void:
	if Globals.get_drag_and_drop().is_dragging():
		if Globals.get_drag_and_drop().data.has("inventory"):
			...
			if inventory.slots[_slot].item_instance != null:
				var item_instance: ItemInstance = inventory.slots[_slot].item_instance
				var amount: int = inventory.slots[_slot].amount
				if item_instance.item == inventory_slot.item_instance.item and item_instance.item.stackable and amount + inventory_slot.amount <= item_instance.item.stack_amount:
					inventory.change_slot(_slot, inventory_slot.item_instance, inventory_slot.amount + amount)
					previous_inventory.change_slot(inventory_slot.index, null, 0)
				else:
					inventory.change_slot(_slot, inventory_slot.item_instance, inventory_slot.amount)
					previous_inventory.change_slot(inventory_slot.index, item_instance, amount)
			else:
				inventory.change_slot(_slot, inventory_slot.item_instance, inventory_slot.amount)
				previous_inventory.change_slot(inventory_slot.index, null, 0)
			Globals.get_drag_and_drop().stop_dragging()
		if Globals.get_drag_and_drop().data.has("gear"):
			...
			if inventory.slots[_slot].item_instance == null:
				Globals.get_player().unequip_gear_in_slot(gear.item.slot)
				inventory.change_slot(_slot, gear, 1)
			else:
				var inventory_gear = inventory.slots[_slot].item_instance.item
				if inventory_gear is Gear and inventory_gear.slot == gear.slot:
					Globals.get_player().equip_gear_in_slot(gear.slot, inventory.slots[_slot].item_instance)
					inventory.change_slot(_slot, gear, 1)
			Globals.get_drag_and_drop().stop_dragging()
