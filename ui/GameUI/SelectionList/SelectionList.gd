...
@onready
var prompt: Label = $CenterContainer/VBoxContainer/Prompt

@onready
var list: VBoxContainer = $CenterContainer/VBoxContainer/ScrollContainer/MarginContainer/List
...
func _ready() -> void:
	...
	item_tooltip.should_be_visible = false
		...
		for list_entry in selection_list:
			...
			selection_list_entry.focus_entered.connect(func():
				if list_entry.data is ItemInstance:
					...
					item_tooltip.should_be_visible = true
