...
func spawn_slime(location: Vector2 = Vector2.ZERO) -> void:
	...
	if location == Vector2.ZERO:
		var query = PhysicsRayQueryParameters2D.create(
			player.global_position + player.global_position.direction_to(slime_position).normalized() * 5,
			slime_position)
		var result = space_state.intersect_ray(query)
		if result:
			slime_position = result.position + result.position.direction_to(global_position).normalized()
